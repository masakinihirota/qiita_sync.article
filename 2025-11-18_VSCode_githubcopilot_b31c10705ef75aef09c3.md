...
自動にする場合は、例えばTDDでモデル別にレッドやグリーンをモデルごとに連続で作って貰う場合に活用できます。
そして出来上がったコードを比較検討するなどの使い方が考えられます。


:::note info
💡ハンドオフ機能をつかい自動で渡すと便利な例💡
※ハンドオフとはバトンリレーというイメージです。

テスト駆動開発でのグリーンを目的としたテストファイルをバトンリレーして一括で作成します。
そして、ここでバトンリレーを止めてからまとめて評価＆レビューをします。

実行例

1. レッド 作成後、sendをtrueでバトンを設定します。
1. グリーン(GPT-4.1)→グリーン(GPT-5 mini)→グリーン(Grok Code Fast 1)→グリーン(Gemini 2.5 Pro)→グリーン(GPT-5-Codex)→グリーン(Claude Sonnet 4.5) を複数同時に作ります。
1. sendをfalseに設定してバトンリレーの自動化をここで止めます。
1. 比較＆レビューをAIにしてもらったり 自力で調べます。
1. 最も良いものを、次にバトンで渡します。

こうやって複数のAIモデルで作成して生成されたテストファイルを比較、レビューをして最適なものを次にまたバトンで渡していきます。

※こうなると並列で動かす仕組みが欲しくなります。
...
✋️ちょっと待って、なぜ複数のAIモデルでコードを書かせるの？
最新最高のAIモデル１つで十分じゃないの？

・・・と思う方👉️

XユーザーのKosukeさん: 「AIエージェントの開発において、最先端のAIモデルは不要です。 不完全なAIモデルを使い「100万ステップ・ゼロエラー」を達成するという衝撃の論文が登場しました。 LLMのハルシネーションを考えると統計的に不可能な偉業です。 その衝撃な論文の詳細を5つのポイントにまとめました。 1. https://t.co/x19oWOWrHz」 / X
https://x.com/kosuke_agos/status/1991076641645420905

自分はこれを複数のAIモデルにコードを出力させ協議させればいいのではないかと推論を立てました。
新世紀エヴァンゲリオンに登場するスーパーコンピュータ「MAGI（マギ）システム」のように。

MAGIシステムとは？
3つの人工知能「メルキオール」「バルタザール」「カスパー」が、合議制で意思決定を行うシステムです。﻿

> 分解と投票
TDD(テスト駆動開発)はまさに分解(最小のタスクリスト化)と投票(ハンドオフ機能で複数コードの生成し、レビューし、その中から選ぶ)を行うのをしやすい開発手法だと思います。


AIモデルは最新が最高ではないという例、現在(2025年11月) Gemini3.0が登場し盛り上がっていますが、ハルシネーション(幻覚)率は👇️従来のモデルとそれほど変わりません。

https://x.com/slow_developer/status/1991252642379751861

![AA-Omniscience Hallucination Rate.jfif](https://qiita-image-store.s3.ap-northeast-1.amazonaws.com/0/44761/923d67e6-e999-474c-9d13-9394fe13449e.jpeg)
