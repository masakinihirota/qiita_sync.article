...
正直読みにくいです。あのマニュアルは開発者視点で書かれていて、開発した機能別に書かれているようです。
...
↑のジェネレーターの setGenerator() をコピーして、2個目をコピーして↓名前だけ変えました。
...
↑名前部分の controller を controller2 に変更してコピーしています。
...
※名前を変えないと、同じジェネレーターとして扱われます。
選択肢は出ずに、１個の時のジェネレーターと同じように実行されます。
