...
## plants.md とは？
plants.md はどんどん更新されいく生きた文書
基本方針は、完全なる初心者だからすべてを教えてくれ
そして、それを教えるのがただ唯一のファイルplans.md
...
## 複雑な2つ目の機能を実装する
新しくplains.mdを用意します
plains.mdはまた初心者の状態から始まります。
GPT-5-Codexはプロジェクト全体を見て、機能を実装する計画をまた1から作ります。
※これはplains.md自体が肥大化するのを避けるためだと思われます。
どのタイミングで2つ目のplains.mdに移動するのか？最後まで使い続けるのかの言及はありません。

## 計画ファイル(plants.md)が複数ある場合の対応
エージェントに実装させる計画を記述します。（ファイル名が`plans.md`の想定）
`AGENTS.md`内で、計画を名前で指定します。具体的には、エージェントに対して、実装すべき計画を名指しで指示します。
また、計画を使用するタイミングを記述し、計画の略称（例: ExecPlan）を定義します。
この略称は任意ですが、GPT-5-Codexに特定の計画の定義を参照させるようプロンプト内で指示するために使用します。

## 目的を教えたい(機能の実装など)
作業の目的と意図が最初にきます。ユーザーの視点から、その変更がなぜ重要か、何ができるようになるか、そして動作していることをどのように確認できるかを数文で説明します。

## 具体的なステップ
編集する場所、実行するコマンド、観察すべき結果など、正確なステップで読者（エージェント）を導きます。
...
### 実際の流れ
AIと組んでペアプログラミングのように行う

人間が大まかに計画を立てる 調査と初期計画
AIが実行 マイルストーン
相談しながら計画を更新(人間もplains.mdを直接編集します)
これを繰り返す
...
plains.mdを使用するタイミング（複雑な機能の作成や大規模なリファクタリング時）と、計画の定義を示す用語（例：「ExecPlan」）を記述することで、これらの文書を使用するように指示します。
