...
ピン止めすると、そのファイルを閉じても、再度開いた時にピン止めしたファイルは開いた状態になります。
...
コードとテストコードを同一ファイル内において開発できるということは、AIにとっても有利なのではないかとおもいます。
何しろAIが見ている範囲が確実にそのファイル内に収まっているのですから、いちいちテストファイルをVSCodeのタブで開く必要がないので、コードを書くハードルが下がります。**copilotにテストを提案させて挿入ボタンを押すだけ**です。
...
AIに聞いてみると、この機能は他のテストツールでも同じようなのがあるようです。
