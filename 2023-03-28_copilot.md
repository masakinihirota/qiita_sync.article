...
# その他 これより下は Github Copilot とは直接関係がない記事です。

例えば、
**Github Copilot chat** だったり、その他のAIの紹介だったりします。
...
これまではBingを使うにはMicrosoft Edgeを起動するか、あるいはブラウザ拡張機能でBiアクセスする必要がありました。

しかし、ブラウザを起動しなくても単体でBing Chatにアクセスできるツール「BingGPT」が公開されました。

## Duet AI for Google Cloud

Google Cloud でのシステム開発について何でも AI が教えてくれる「Duet AI for Google Cloud」発表。Google I/O 2023 － Publickey
...
次世代の Github Copilot X シリーズを開発、発表している場所です。
...
githubnext は次世代の **GitHub Copilot X** の開発元です。
