...
※提案とはGitHub Copilotが出してくるコードの事です。
...
次に拡張機能を利用するためにGitHubにログインするよう求められます。
ログインをします。
...
#### まとめて提案を出す
通常は１行に1つづつ、その他の次の提案はショートカットキーで一つ一つ見ていきますが、それをまとめて表示して欲しい時に使用します。
...
`type User`と入力した時点でCtrl＋Enterを押すと、幾つかの提案が表示されます。
...
※関数名がその関数の説明文になっているのがポイントです。
※閉じ括弧は習慣としてつけておいた方がよいでしょう、提案から選ばない可能性もあるので。
VSCode機能で自動的に閉じ括弧がつくように設定しておくと便利です。
