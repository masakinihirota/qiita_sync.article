...
例えば、一つ上の ** Make this code easier to debug ** で追加されたconsole.logは本来の関数には不要なので削除してもらいます。
...
機能: コードにJSDocを追加します。（ドキュメントのアイコン）
...
かと言ってコメントをぎっしり書いてもまだまだAIは発展途上なので、完全に製作者の意図を汲み取ってくれるわけではありません。GPT-4,5,6に期待しましょう。
...
code does following (English)
Show example code
これらの **Advanced** を変更すると、すべて **Custom** に強制移動します。
...
単にテストコードと
テストフレームワークを書くだけでは動きません。

テストコードは自動生成してくれますが、
実行させてみるとエラーばかり返ってきて動きません。

(なので調査中)

テストコードを書いたテストファイルとテストフレームワークをインストールするだけでは動きません。
