...
自動挿入機能を持つ拡張機能は、GitHub Copilot と競合するため、OFFにしてください。

具体的な GitHub Copilot と相性の悪い拡張機能
...
優先順位が 3 番目の会話ログなので忘れた言動のような答えになるらしい。

※注意点トークンを超える情報は参考されない。
10 も 20 もファイルを開いていても情報過多で参考にされない。
(参考:トークン ChatGPT だと英文字 4096 日本語だと 3000 文字ぐらい、GitHub Copilot の許容トークン数は不明)
