...
業務ではMicrosoft AzureやGitHubの技術アドバイザリやワークショップの講師を担当。
Twitter:@dz
...
優先順位は
1. 質問
1. 今、VSCode上で範囲選択をしている部分
1. 今、開いていて見ているファイルの全体
1. 会話ログ
1. タブで開いているが、画面上には表示されていないファイル
みたいな感じだと思います。

例
一つ前の質問とその答えも忘れてるっぽい言動もみられる
優先順位が3番目の会話ログなので忘れた言動のような答えになるらしい。

※注意点トークンを超える情報は参考されない。
10も20もファイルを開いていても情報過多で参考にされない。
(参考:トークン ChatGPTだと英文字4096 日本語だと3000文字ぐらい、GitHub Copilot の許容トークン数は不明)
...
GitHub Copilotを使ったコードリーディングのコツ その1 - Qiita
...
このような工夫でAIにコードリーデングの手伝いを実現させています。
