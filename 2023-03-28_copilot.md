...
コードとテストコードを同一ファイル内において開発できるということはAIにとっても有利なのではないかとおもいます。
何しろAIが見ている範囲が確実にそのファイル内に収まっているのですから、いちいちテストファイルをVSCodeのタブで開く必要がないので、コードを書くハードルが下がります。copilotにテストを提案させて挿入ボタンを押すだけです。
