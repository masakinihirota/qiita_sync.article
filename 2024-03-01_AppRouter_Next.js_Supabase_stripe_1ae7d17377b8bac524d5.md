...
↑上の記事はこの記事を書いた後、デプロイボタンを利用してパターン1のみを設定した記事になります。(エラーも修正されています。)

↓下の記事ではStripeの設定で説明がない所(エンドポイントを追加)がありました、そのせいで一部失敗することがわかっています。
(失敗例：商品を購入するところでエラーになる)
Stripe方でイベントリスナーを設定すると、Stripeの画面でイベントリスナーでのURLが表示されれば正常に動きます。

↓環境変数のパターン1,2,3とも設定は正常ですが、Stripe側の設定(エンドポイントを追加)が足りていません。
