...
例えば10年前の、いえ3年前のセキュリティ対策でも今では通用しない場合があります。
...
Authenticationで大変重要で根底にあるのは、認証と認可の違いを理解することです。
...
自分の部屋に「入る鍵」が認可に当たります。
自分の部屋の中で食事をしたり、寝たり、冷蔵庫から物を取り出したり自由に出来ます。
...
データベースはエクセルのファイルを開いたような状態です。
エクセルのように自由ではなく、どのような場所にどのような情報を書くかが決まっています。
...
ルールエンジンと言われてもピンと来ないかもしれませんが、
部屋の入室許可を自分だけにするか、家族全員にするか、友人にも許可するか等を決めるのがポリシーです。
...
3. Multi-Factor Authentication (マルチファクタ認証)
...
identitiesテーブルは、Supabaseの認証機能に関するユーザーのアイデンティティ情報を管理するために使用されます。
