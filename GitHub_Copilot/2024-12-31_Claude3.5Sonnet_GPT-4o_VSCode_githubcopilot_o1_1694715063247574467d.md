...
プロンプトファイルはタスクや機能や、目的ごとのプロンプトを作成して、AIに指示できるファイルです。

`copilot-instructions.md` ファイルが GitHub Copilot への全体設定ならば、
プロンプトファイルは、GitHub Copilot に個別に指示するファイルです。

のような指示を、目的別に細かく分けることができる方法です。

※ 最近 `copilot-instructions.md` ファイルが長くなってきたので複数ファイルに分割することで目的別に管理しやすくするつもりで導入されたのではないでしょうか？

イメージとしては
静的 `copilot-instructions.md` ファイル (共通のルール)
動的 プロンプトファイル (個別、目的別のルール)
