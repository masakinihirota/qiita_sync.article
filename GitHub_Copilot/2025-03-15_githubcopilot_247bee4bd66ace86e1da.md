...
指示書に書いてない部分はGitHub Copilotが自由にコードを生成してくる場合があります。
...
英語では custom instructions file と呼ばれます。
...
設計書に基づいて分割された、実装すべき機能単位のリスト。各タスクは、コード実装を優先して定義されます。
...
実装書は設計書をタスクリストかからタスク分解をして、特定の目的や機能を詳細に書いたものです。
...
AIとチャットで会話して、会話が終了するまでの間だけ覚えておくルールというイメージ。
...
設計書からタスクリストを作成し、GitHub Copilotが処理しやすいようにタスク分解し、GitHub Copilotの指示書を作成します。その指示書を実装書として管理します。
...
各タスクの詳細な仕様、バージョン情報など、AIへの指示を記述します。
これらの情報を「実装書」にまとめます。
実装書はタスクリストの各項目に対応する1機能単位で分けてAIに読み込ませます。

4. AIによる実装

作成したテンプレートと実装書をAIに読み込ませ、機能の実装をAIに行わせます。
...
設計書と実際のコーディングの中間段階として、AIに対する詳細な指示書となります。
機能単位で分割されたタスクと、その実装に必要な情報をまとめます。
AIによる実装結果を検証するための基準となります。
AIとの円滑な連携を可能にするための重要な架け橋となります。

## 実装書のポイント

人間の手による制御のため、人間とGitHub Copilotとのバランスを考えます。
開発者は設計とタスク管理に集中し、AIは実装に特化させることで、効率的な開発を実現します。
詳細な実装書を作成することで、AIによる実装の精度と効率を高めます。
