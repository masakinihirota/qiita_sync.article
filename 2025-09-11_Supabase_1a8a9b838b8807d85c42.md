...
このコマンドは、リモートのSupabaseデータベースから`backup.backup`というファイルに出力します。
...
*   **ロール（ユーザー・権限）のみ**: `supabase db dump --role-only -f backupRoles.backup`
*   **特定のスキーマ（例: `public`と`auth`）のみ**: `supabase db dump -s public,auth -f backupPartial.backup`
*   **データのみ**: `supabase db dump --data-only -f backupData.backup`
...
$filename = "backupAll_local_$timestamp.backup"
$backupDir = "backup"
