...
private: false
-->
Supabaseを利用している皆さん、データのバックアップは万が一の事態に備えるためのとても大切な作業です。この記事では、Supabaseの全データをバックアップ・リストアする方法から、日々の自動バックアップ、そして役立つアドバイスまで、具体的な手順を交えてご紹介します。
...
`supabase db execute`コマンドを使ってSQLファイルをリモートDBに適用するのが一般的です。

#### リストア手順
1.  **ロール（ユーザー・権限）のリストア**
    まず、ロールのみをバックアップしたSQLファイル（例: `backuproles.sql`）をリストアします。
    `bash
    supabase db execute --file backuproles.sql
    `
    これは、以前に`supabase db dump --role-only -f backuproles.sql`で作成したファイルを使用します。

2.  **全体（スキーマ＋データ）のリストア**
    次に、スキーマとデータの両方を含むSQLファイル（例: `backupAll.sql`）をリストアします。
    `bash
    supabase db execute --file backupAll.sql
    `
    これは、`supabase db dump -f backupAll.sql`で作成したファイルを使用します。

**【重要】** **必ず「ロール」→「全体（スキーマ＋データ）」の順で実行してください**。この実行順序を守ることで、エラーを防ぐことができます。

**【注意点】**
*   `supabase db execute`は **リモートDB** に対して実行されます。
*   **ローカルDB**をリストアする場合は、`--local`オプションを追加してください。
*   **大きなファイルや大量データの場合は、`psql`コマンドを使う方が高速で確実な場合もあります**。
