<!--
title:   Drizzle 公式ドキュメント with Supabase Next.js 15 (仮)
tags:    Drizzle
...
Supabaseはローカルとサーバーを用意します。

そのSupabaseはローカルで開発とテストを、サーバーで運用をします。

DrizzleはローカルのSupabase DBを、TypeScriptのコードで管理してもらうツールになります。

AI を利用して開発のサポートをしてもらいます。
...
* ポリシー データベースのテーブルに対するアクセス制御を細かく設定します。 簡単に言うと、誰がどんな操作をできるかをルールで決めます。
...
* PostgreSQLのスキーマは、データベースオブジェクトを論理的にグループ化するための仕組みです。スキーマは名前空間のような役割を果たし、同じ名前のテーブルが異なるスキーマに存在することを可能にします。Drizzle ORMでは、pgSchema 関数を使用してスキーマを作成し、そのスキーマ内にテーブルを定義できます。
...
＜＜＜＜役割を演じてください 役割 PostgreSQLの専門家 優しく日本語で簡単な解説をしてください。
