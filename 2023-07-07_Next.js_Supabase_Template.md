...
# リポジトリからのインストール（プライベートリポジトリ）

**※現在プライベートリポジトリにしてあるのでリポジトリページを開けません。**
...
import 文の自動削除や自動挿入は保存ボタンを押すと実行されます。（VSCode 側で Prettier が自動実行の設定がされている場合）

(1 度で動かない時は 2,3 回保存ボタンを押してみてください。)
...
※現在の問題点、Storybookをインストールすると、ESLintの設定が効かなくなる。
StorybookのESLint設定を諦める、ファイルを直接指定してフォーマットをする、Storybookをインストール時にESLintを除外する等が考えられる。



npx storybook@latest init

インストールが完了すると Storybook のチュートリアルが始まるので体験してみてください。
...
Radix × Tailwind な shadcn/ui がいい感じ - Qiita
...
shadcn/ui の UI コンポーネントを追加
npm dlx shadcn-ui@latest add dropdown-menu

```globals.css
※ globals.css は shadcn/ui インストール時に設定されたのをそのまま利用します。

```

```tailwind.config.js
※ tailwind.config.js は shadcn/ui インストール時に設定されたのをそのまま利用します。

```

theme provider を作成します。
...
ライトモードとダークモードを切り替えるトグルをサイトに設置します。
...
globals.css の CSS 変数を変更することで、テーマごとの配色を一元管理できます。

さらに、text-muted-foreground/50 等の透過調整や sm:text-muted-foreground などの装飾子も使用可能になります。

また、カスタムカラーとして登録されるため、Tailwind CSS Intellisense で色の指定の競合をハイライトしたり、色の候補としてラインナップしてくれるようになります。

注意点として、text-gray/50 などのように色の透過度を調整可能にするために、色はカラーチャネルのみで定義する必要があります。

RGB や HSL のフォーマットに応じて、tailwind.config.js 側でカスタムカラーと紐づける際の記述が変わる点にも注意しましょう。

## Next.js でのダークモード環境設定

Next.js インストール時に Tailwind CSS を Yes 選択します。
...
## テーマ設定にユーティリティクラスを使用するには、components.json ファイルで tailwind.cssVariables を false に設定します。
