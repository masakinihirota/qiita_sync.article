...
i18nを調べたことをまとめましたが、Qiitaでは容量的に受け付けてもらえなかったので、こちらにおいておきます。

i18n 国際化フレームワーク i18next入門

https://github.com/masakinihirota/zenn_sync.article/blob/main/2023-09-07_AppRouter_I18n_Next.js_i18next_4a99d571dea7b9d3a490.md
...
例えば、pagesやappと同じレベルか、該当する場合はsrcの中です。
...
このミドルウェア関数は、`export`されているため、他のファイルからインポートして使用することができます。また、`config`オブジェクトを定義することで、このミドルウェア関数をどのようなリクエストに対して適用するかを設定することができます。

このように、ミドルウェアを使用することで、リクエストに対する処理を柔軟にカスタマイズすることができます。
...
リクエストに対して条件文で２種類のURLの書き換えを行っています。
