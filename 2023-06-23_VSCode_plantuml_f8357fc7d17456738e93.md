...
PlantUML - Simple Viewer v2.17.6 をVSCodeにインストールします。


PlantUML    v2.17.5 well-ar 1,557,773   ★★★★★  (93)    2022/12/12 11:50:29
...
機能
* プレビュー図の表示。PlantUMLプレビューを開始するには、Alt + D（MacOSではOption + D）を押します。
* 自動更新。
* ズームとスクロールのサポート。
* 複数ページの図のサポート。
...
* 境界線にスナップ。
* 図のエクスポート。カーソル位置、現在のファイル、ワークスペース全体、選択したワークスペースでエクスポートできます。
* 同時エクスポート。
* URLの生成。
* 複数ページの図のサポート。
* ローカルまたはサーバーからのエクスポート。
...
※タブでインデントを下げているのは使えそうだけど自分の好みではないもの。
...
超入門 ハンズオン PlantUML で遷移図 ER図 for VSCode - Qiita
