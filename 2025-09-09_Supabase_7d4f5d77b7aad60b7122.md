<!--
title:   ９月末までに Supabase 非対称キー JWT Signing Keys (JWT署名鍵 getUsersからgetClaimsに)に変更
tags:    Supabase
...
既存のアプリでは
`getUsers()` 部分を `getClaims()` に変えるだけで移行は完了です、ただしこのままでは性能は従来通りです。
...
つまり、getUsersの機能を受け継ぎつつ、アプリケーションでも検証できる新しい機能が追加されています。
