<!--
title:   Supabase JWT Signing Keys に変更 (JWT署名鍵 非対称キー)
tags:    Supabase
...
https://supabase.com/changelog?next=Y3Vyc29yOnYyOpK0MjAyNC0wOS0xM1QxNTowMDoyOVrOAG2JYw==&restPage=2

### Supabase APIキーの今後の変更 2024年9月13日

この変更は早期プレビューとして開始され、オプトイン制です。
少なくとも2025年11月1日までは、特別な対応は必要ありません。新しいAPIキーをぜひお試しください。



2025年11月	従来のAPIキーから新しいキーへの移行をお願いするリマインダーを毎月お送りします。

以降に復元されたプロジェクトは、従来のAPIキーでは復元されなくなります。新しいプロジェクトには、以前のAPIキーは適用されず、ご利用いただけなくなります。

anonservice_role	一時停止中のプロジェクトを復元すると、従来のキーがないため壊れるリスクがあるため、この日付までに新しい API キーを使用するよう移行することを強くお勧めします。

2026年後半（未定）
レガシー API キーは削除され、ドキュメント/ダッシュボードから削除されます。
この時点で新しい API キーを使用するように移行する必要があります。そうしないと、アプリが壊れてしまいます。
